...
const Approach = () => {
  ...
  const sampleReceipts = [
    { name: "Office Supplies", amount: 234.56, date: "2024-03-15" },
    { name: "Client Lunch", amount: 89.99, date: "2024-03-14" },
    { name: "Software License", amount: 499.99, date: "2024-03-13" },
  ];
      ...
      widget: (
        <div className="glass-card p-6 rounded-xl relative overflow-hidden group">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="border-2 border-dashed border-billsync-accent/30 rounded-lg p-6 text-center relative z-10"
            ...
            {isUploading ? (
              <div className="space-y-4">
                <div className="w-full bg-billsync-accent/20 rounded-full h-2">
                  <motion.div 
                    className="bg-billsync-accent h-2 rounded-full"
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-center gap-3">
                  {['.pdf', '.png', '.jpg'].map((format, index) => (
                    <span key={index} className="text-xs px-2 py-1 rounded-full bg-billsync-accent/10 text-billsync-accent">
                      {format}
                    </span>
                  ))}
                </div>
      ...
      widget: (
        <div className="glass-card p-6 rounded-xl">
          <div className="space-y-6">
            ...
            >
              {[
                { label: "Receipts Processed", value: "1,234" },
                { label: "Categories", value: "15" },
                { label: "Total Amount", value: "$45,678" },
                { label: "Tax Collected", value: "$3,456" },
              ].map((stat, index) => (
                <motion.div
                  key={index}
